...
import { EdgeLabelRenderer, useViewport, Viewport } from "reactflow";
...
export function Cursor({ x, y, viewport, info }: CursorProps) {
  const cursorPath = `
  ...
  return (
    ...
    >
      <g style={{ transform: scale }}>
        ...
        <text y="11px" x="17px" fontSize="10px" fill={color}>
          {info.name}
        </text>
      </g>
    </svg>
  ...
  return (
    <EdgeLabelRenderer>
      {others.map(({ connectionId, presence, info }) => {
        if (presence.cursor && !presence.selectedNodeId) {
          const { x, y } = presence.cursor;
          return <Cursor key={connectionId} x={x} y={y} viewport={viewport} info={info} />;
        } else {
          return null;
        }
      })}
